...
page_fault (struct intr_frame *f) 
{
  ...
  void *fp_addr = (void*) pg_round_down(fault_addr);
  void *st_ptr = f->esp;

#define MAX_STACK_SIZE 0x800000

  if(fault_addr>=PHYS_BASE-MAX_STACK_SIZE && fault_addr<PHYS_BASE){
	if(st_ptr-fault_addr < MAX_STACK_SIZE && (st_ptr<=fault_addr || fault_addr==st_ptr-4 || fault_addr==st_ptr-32)){
		stack_growth(fp_addr, st_ptr);
		return;
	}
	else exit(-1);
  }
